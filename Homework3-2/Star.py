...
class Star():
    ...
    def loadStarData(self):
        ...
        for newpoint in range(numpoints):
            oPoints.append([(self.atts.outerR * np.cos(newpoint * 2 * np.pi / numpoints + np.pi / 2)+ self.atts.cx),
                            (self.atts.outerR * np.sin(newpoint * 2 * np.pi / numpoints + np.pi / 2)+ self.atts.cy)])
            colors.extend(self.atts.color)
            iPoints.append([(self.atts.outerR / 3 * np.cos(newpoint * 2 * np.pi / numpoints + np.pi / numpoints + np.pi / 2 ) + self.atts.cx),
                            (self.atts.outerR / 3 * np.sin(newpoint * 2 * np.pi / numpoints + np.pi / numpoints + np.pi / 2 ) + self.atts.cy)])
