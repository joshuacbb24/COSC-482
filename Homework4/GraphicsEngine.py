...
class GraphicsEngine():
    ...
    def __init__(self):
        ...
        self.r = glGetUniformLocation(self.shaderProgram, "r")
        self.g = glGetUniformLocation(self.shaderProgram, "g")
        self.c = glGetUniformLocation(self.shaderProgram, "change")
        self.red = 1.0
        self.green = 0.0
        self.change = 0
        glUniform1f(self.r, self.red)
        glUniform1f(self.g, self.green)
        glUniform1i(self.c, self.change)
    ...
    def fade(self):
        now = time.time()
        t = now - self.starttime2
        glUseProgram(self.shaderProgram)
        if self.change == 0:
            self.red = self.red - .5 * t
            self.green = self.green + .5 * t
        else:
            self.red = self.red + .5 * t
            self.green = self.green - .5 * t
        if self.red <= 0.0:
