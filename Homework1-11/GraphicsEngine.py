...
class GraphicsEngine():
    ...
    def incrementPoints(self, inc):
        if (2 < self.points < 100) or (self.points == 2 and inc == 1) or \
            ...
            for newpoint in range(self.points):
                oPoints.append([(self.oRadius * (np.cos(((newpoint * 2 * np.pi) / self.points) + np.pi / 2))),
                                (self.oRadius * (np.sin(((newpoint * 2 * np.pi) / self.points) + np.pi / 2)))])
                iPoints.append([(self.oRadius / 3 * np.cos(
    ...
    def incrementRadius(self, inc):
        if self.oRadius < 1:
            self.data = []
            oPoints = []
            iPoints = []
            counter = 0
            self.oRadius = self.oRadius + inc
            for newpoint in range(self.points):
                oPoints.append([(self.oRadius * (np.cos(((newpoint * 2 * np.pi) / self.points) + np.pi / 2))),
                                (self.oRadius * (np.sin(((newpoint * 2 * np.pi) / self.points) + np.pi / 2)))])
                iPoints.append([(self.oRadius / 3 * np.cos(
